...
echo -e "\n🦊 TẠO CẤU HÌNH PIPELINE CI/CD ĐA CHI NHÁNH CHO FRONTEND Next.js..."
echo -e "------------------------------------------------------------------------"
...
deploy-${br}:
  ...
  image: alpine:latest
  rules:
    - if: \$CI_COMMIT_BRANCH == \"${br}\"${deploy_dep}
  before_script:
...
# ─── In ra màn hình ────────────────────────────────────────────────────────────
echo -e "\n${BOLD}${CYAN}========================================================================${NC}"
echo -e " 📋 ${BOLD}${WHITE}NỘI DUNG FILE .gitlab-ci.yml (FRONTEND)${NC}"
...
echo -e "${BOLD}${CYAN}------------------------------------------------------------------------${NC}"
echo -e "\n${OK} Đã tạo cấu hình GitLab CI/CD Frontend thành công!"
...
for br in "${BRANCHES[@]}"; do
    br_upper=$(echo "$br" | tr '[:lower:]' '[:upper:]' | tr '-' '_' | tr '/' '_')
    echo -e "       - ${BOLD}ENV_LOCAL_${br_upper}${NC} (File) → nội dung .env.local"
done
